...
from MyImageIdentifier.pixel_classifier_v3.prepare_a2d2_pc_ds import (
    convert_mask , convert_road_mask , label_key , prepare_dataset , split_scenes
)


class PrepareA2D2Tests(unittest.TestCase):
    def test_v3_keeps_v2_train_and_test_structure(self):
        root = Path(__file__).resolve().parent.parent
        trees = []
        for version in ("v2" , "v3"):
            source = (root / ("pixel_classifier_" + version) / "train.py").read_text(encoding = "utf-8-sig")
            source = source.replace("pc_model_v3.pth" , "pc_model_v2.pth")
        ...
        original = (root / "pixel_classifier_v2/test.py").read_text(encoding = "utf-8-sig")
        current = (root / "pixel_classifier_v3/test.py").read_text(encoding = "utf-8-sig")
        current = current.replace("pixel_classifier_v3" , "pixel_classifier_v2").replace("pc_model_v3.pth" , "pc_model_v2.pth")
