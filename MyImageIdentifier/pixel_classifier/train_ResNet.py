...
ResNet18_encoder = nn.Sequential(
    ...
    nn.MaxPool2d(kernel_size = 3 , stride = 2 , padding = 1) , #112 * 64
    BasicBlock(64 , 64 , stride = 1) ,
    BasicBlock(64 , 64 , stride = 1) ,

    BasicBlock(64 , 128 , stride = 2) , #56 * 32
    BasicBlock(128 , 128 , stride = 1) ,

    BasicBlock(128 , 256 , stride = 2) , #28 * 16
    BasicBlock(256 , 256 , stride = 1) ,

    BasicBlock(256 , 512 , stride = 2) , #14 * 8
    BasicBlock(512 , 512 , stride = 1) ,
)

decoder = nn.Sequential(
    nn.ConvTranspose2d(512 , 256 , kernel_size = 2 , stride = 2) , #28 * 16
    nn.BatchNorm2d(256) ,
    nn.ReLU(inplace = True) ,

    nn.ConvTranspose2d(256 , 128 , kernel_size = 2 , stride = 2) , #56 * 32 
    nn.BatchNorm2d(128) ,
    nn.ReLU(inplace = True) ,

    nn.ConvTranspose2d(128 , 64 , kernel_size = 2 , stride = 2) , #112 * 64
    nn.BatchNorm2d(64) ,
    nn.ReLU(inplace = True) ,

    nn.ConvTranspose2d(64 , 64 , kernel_size = 2 , stride = 2) , #224 * 128
    nn.BatchNorm2d(64) ,
    nn.ReLU(inplace = True) ,
    ...
    nn.ConvTranspose2d(32 , 3 , kernel_size = 2 , stride = 2) #448 * 256
)
...
torch.save(model.state_dict() , "pc_model_ResNet.pth")
print("\nmodel saved: pc_model_ResNet.pth")
...
axes[0].set_title("ResNet Loss Curve")
axes[0].grid(True)
axes[0].legend()
...
axes[1].set_title("ResNet mIoU Curve")
axes[1].grid(True)
axes[1].legend()

figure.tight_layout()
figure.savefig("figure_ResNet.png" , dpi = 300 , bbox_inches = "tight")
plt.close(figure)
print("\nfigure saved: figure_ResNet.png")
