"""PyTorch Dataset for the converted BDD100K road/vehicle masks."""
...
class BDDVehicleRoadDataset(Dataset[tuple[torch.Tensor, torch.Tensor]]):
    """Read image/mask pairs listed in a generated CSV file.

    Returned values:
      image: float32 tensor, shape (3, 256, 448), values in [0, 1]
      mask:  int64/long tensor, shape (256, 448), values in {0, 1, 2, 255}

    The image is resized with bilinear interpolation. The mask is resized with
    nearest-neighbor interpolation and is never normalized or divided by 255.
    """
    ...
    def _resolve_csv_path(self, raw_path: str) -> Path:
        """Resolve a CSV path relative to the CSV's parent directory."""
        ...
        with self.pairs_csv.open("r", encoding="utf-8-sig", newline="") as file:
            ...
            for line_number, row in enumerate(reader, start=2):
                ...
                if stem in seen_stems:
                    raise ValueError(
                        f"CSV 第 {line_number} 行 stem 重复：{stem}"
                    )
    ...
    def __getitem__(self, index: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        unexpected_values = mask_values - ALLOWED_MASK_VALUES
        if unexpected_values:
            raise ValueError(
                f"mask resize 后出现非法类别 {sorted(unexpected_values)}："
                f"{record.mask_path}"
            )
        ...
        # 不归一化 mask；uint8 直接转换成 PyTorch long/int64。
